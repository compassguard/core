...
- [ ] **Post-lease-removal schema cleanup** — after the lease removal (workstream A) is fully deployed out: (1) `DROP COLUMN claimed_at` from the `verdicts` table — it's left nullable-and-unwritten now because a DROP during rolling-deploy overlap would error old instances still running the claim/release `UPDATE … SET claimed_at`; safe once no lease-bearing code runs; and (2) the durable store's `ensureSchema` re-runs the idempotent forward-compat MIGRATIONS (`ADD COLUMN IF NOT EXISTS`) on **every** cold start — gate behind a schema-version check to run once (harmless as-is, just wasteful).
