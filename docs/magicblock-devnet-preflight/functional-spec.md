...
## Wave 14B MCP observer

The real MCP proxy `callTool` wrapper may now observe one successful downstream result after policy and downstream execution have completed. The observer is disabled unless `COMPASS_MAGICBLOCK_MCP_OBSERVER_ENABLED` is exactly `true` and its dedicated HTTPS audit URL plus `COMPASS_MAGICBLOCK_MCP_OBSERVER_API_KEY` are valid. It never reuses the audit-ingress key or general hosted-evaluation credentials.

The wrapper passes only the downstream `CallToolResult.structuredContent` root to the extractor. It accepts exactly `{ schemaVersion: "compass.magicblock-devnet-observation/v1", observationId, unsignedTransactionBase64 }`, with no missing or extra members, an opaque bounded identifier, and canonical base64 decoding to between 1 and 1232 bytes. Canonical base64 includes zero unused padding bits, so alternate forms such as `AB==` and `AAB=` are rejected. The extractor returns a detached frozen observation; the sink never receives the original result or its mutable `structuredContent`. It does not inspect text content, follow nested objects, search arbitrary structures, or observe a denied, approval-required, synthesized, or downstream-error result.

For an accepted envelope, the wrapper normalizes synchronous and asynchronous sink invocation and awaits one independent audit HTTP call with a maximum one-second configured timeout and a 500ms default. Synchronous throws, rejection, late transport rejection, timeout, non-2xx status, malformed observation, and attempted sink mutation are swallowed audit-only and fail-open. The exact original downstream `CallToolResult` object reference and deep state are returned unchanged. The hosted response body is never read and no response value is available to policy, approval, dispatcher, execution, signing, sending, or delegation flow.

The HTTP client accepts only a canonical HTTPS DNS URL at `/api/magicblock-devnet/audit`, rejects credentials, alternate ports, query strings, fragments, IP literals, localhost, and redirects, bounds the serialized request before transport, and authenticates with the dedicated bearer. The local end-to-end test uses linked in-memory MCP SDK transports and crosses `createProxyMcpServer`, request-handler wiring, the real dispatcher, a fake `DownstreamMcpClient`, the real observer/client, an injected hosted-audit transport, hosted ingress, fake MagicBlock provider, and PGlite persistence. It also proves wrong audit authentication is fail-open and creates no observation. No test contacts MagicBlock, Solana, Supabase, Postgres, or another hosted service.

The dependency gate fixes the complete local and external dependency set for each of the seven observer/server roles. Static imports, re-exports, TypeScript import types, CommonJS loaders, dynamic imports, Node runtime module loaders, unresolved paths, and paths outside the source roots are inspected or rejected. Only the explicitly enumerated MCP SDK modules and server Node builtins are permitted. Global capabilities are deny-by-default: each required `process`, `globalThis.fetch`, or `Object.freeze` occurrence must match its exact direct AST form, named-function context, arguments, and expected count. Every computed member access is also deny-by-default; only the seven exact indexed reads currently required by the extractor, config, and server are allowed and counted, while the other four roles allow none. Because the current seven files need no destructuring bindings, every `BindingElement`, object/array destructuring assignment, and object/array `for…in/of` target is rejected. This covers aliased, shorthand, nested, parameter, string-literal, computed-name, rest, and defaulted binding forms, including inherited `constructor` and `__proto__` reads.

Assigning or aliasing globals, indirectly calling their members, computed or reflective retrieval, `Object` reflection, Function-constructor derivation, `process.getBuiltinModule`/`binding`/`dlopen`, Solana packages, `node:child_process`, and unexpected bare dependencies fail closed. These are syntactic dependency/capability restrictions over the seven named source files, not a claim that arbitrary JavaScript is sandboxed.

There is no live endpoint verification, mainnet support, policy decision, signer, sender, submission, delegation, checkpoint, registry write, or strategic activation in this wave.
